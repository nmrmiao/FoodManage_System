...
void CLogindlg::OnOK()
{
    CMyApp *theApp=(CMyApp*)AfxGetApp();//得到app类指针
    UpdateData(TRUE);   //得到用户名与密码
    if(!m_Username.IsEmpty()||!m_Userpasswd.IsEmpty())//判断用户名与密码是否为空
    {
        CString sql="SELECT * FROM LoginInfo WHERE Uname='"+m_Username+"' and Upasswd='"+m_Userpasswd+"'";//SQL语句
        try
        {           
            m_pRs.CreateInstance("ADODB.Recordset");//指针初始化
